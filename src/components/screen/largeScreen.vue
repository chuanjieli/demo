...
export default {
  ...
  mounted() {
    ...
    if (this.$route.params.site == 'world') {
      logo.style.background = 'url(../src/img/china.png) -5px -2px'
    } else {
      logo.style.background = 'url(../src/img/world.png) -5px -2px'
    }
  },
  methods: {
    cutMap() {
    ...
    block(e) {
      if (this.num % 2 == 0) {
        e.target.style.background =
          "url('../src/img/arrow.png') no-repeat center center"
        this.$refs.listBody.style.display = 'none'
      } else {
        e.target.style.background =
          "url('../src/img/arrow1.png') no-repeat center center"
    ...
    getData(site) {
      this.$http
        .get('http://10.1.4.52:8000/hedunwaf/' + site + '_all_info/')
        .then(res => {
          if (res.data.msg == 'ok') {
    ...
    getData1(site, id) {
      ...
      this.$http
        .get(
          'http://10.1.4.52:8000/hedunwaf/' + site + '_attack_logs/?id=' + id
        )
        .then(res => {
          var that = this
          if (res.data.msg == 'ok') {
